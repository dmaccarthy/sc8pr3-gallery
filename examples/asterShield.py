...
Use the arrow keys to accelerate; A/D to turn; S/W to stop.
"""
...
from sc8pr.geom import vec2d, delta, sigma
...
class Ship(Sprite):
    ...
    original = Image.fromZip("ship", "img.data")
...
class Missile(Sprite):
    original = Image.fromZip("missile", "img.data")
...
class Asteroid(Sprite):
    original = Image.fromZip("target", "img.data")

    def __init__(self, sk):
        "Create a new asteroid"
        super().__init__(Asteroid.original)
...
class Game(Sketch):
    ...
    def setup(self):
        ...
        # Create ship sprite
        self.player = Ship(Ship.original).config(wrap=BOTH)
    ...
    def ondraw(self, ev=None):
        ...
        if p in self: # Game in progress
            ...
            # Capture video frame
            if CAPTURE and self.frameCount % 2: self.capture.write(self)
    ...
    def drawHighScores(self, score):
        ...
        okay = Text("Okay").config(font=Font.sans(), fontSize=16)
        self += Button((w/6, h/8)).bind(onmousedown=restart).textIcon(okay,
...
class PlayerName(TextInputCanvas):
    "GUI control for obtaining the player's name"

    def __init__(self):
        super().__init__(320, "", "Please enter your name", font=FONT, color="red", promptColor="black", padding=8)
        self.bg = "white"
...
CAPTURE = 0 * "asteroid.mp4"

def play(size=(608, 400)):
    print(__doc__)
    sk = Game(size).config(imgFldr=resolvePath("img", __file__))
    if CAPTURE:
        sk.capture = ffr = Writer(CAPTURE, 30)
        ffr.interval = 2
    try: sk.play("Asteroid Shield", Asteroid.original)
    except: pass
    if CAPTURE: ffr.close()
